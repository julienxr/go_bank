...
func TestListAccounts(t *testing.T) {
	...
	accounts1, err := testQueries.ListAccounts(context.Background(), arg1)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Len(t, accounts2, 0)

	// limit and offset boundary check
	...
	require.NoError(t, err)
	require.Len(t, accounts3, 0)

	// offset exceeds total
	arg4 := ListAccountsParams{
		Limit:  5,
		Offset: 500, // assumption is less than 500 records...
	}

	accounts4, err := testQueries.ListAccounts(context.Background(), arg4)
	require.NoError(t, err)
	require.Len(t, accounts4, 0)
}
